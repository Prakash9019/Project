...
import { confirmPhoneOtp, getPendingPhone, sendPhoneOtp, clearPhoneAuth } from '../../src/services/phoneAuth';
...
export default function PhoneCode() {
  ...
  const verify = async (fullCode: string) => {
    ...
    setError(null);
    try {
      const idToken = await confirmPhoneOtp(fullCode);
      ...
      if (err.code === 'auth/invalid-verification-code') setError('That code is incorrect. Try again.');
      else if (err.code === 'auth/code-expired') setError('That code expired. Request a new one.');
